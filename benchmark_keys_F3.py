"""
benchmark_keys_F3.py — Benchmark multi-clés sur STM32F3, Config H (Tout en CCM).
Objectif : évaluer si la clé AES influence la résistance à l'attaque CPA.
Usage : source ~/.cwvenv/bin/activate && python3 benchmark_keys_F3.py
"""
import sys, os
...
from cpa_core import (
    ...
    RESULTS_DIR, FW_PATH, plot_guessing_entropy, print_summary
)
...
# ─── PHASE 1 : Compilation une seule fois (Config H = CCM) ─
print(f"\n{'='*60}")
print(f"  PHASE 1 : COMPILATION CONFIG H Tout en CCM")
print(f"{'='*60}")
...
try:
    for run_idx in range(1, N_RUNS + 1):
        ...
        # Flash une seule fois par run (même firmware Config H pour toutes les clés)
        program_target(scope, target, PLATFORM)

        for key_cfg in KEYS:
            print(f"\n  [{key_cfg['name']}] {key_cfg['label']}")

            # Capture avec la clé courante
            # On injecte la clé dans capture_traces via KNOWN_KEY override
            import cpa_core
            ...
            cpa_core.KNOWN_KEY = _orig_key  # restore
...
# ─── RÉSUMÉ ─────────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"  RÉSUMÉ FINAL — {N_RUNS} runs — Config H — {PLATFORM}")
print(f"{'='*60}")
...
for key_cfg in KEYS:
    ...
    if checkpoints_ref is not None and len(ranks_data[k]) == N_RUNS:
        ...
        plot_guessing_entropy(
            ...
            filename=f"guessing_entropy_{PLATFORM}_configH_key{k}.png"
        )
...
if checkpoints_ref is not None:
    ...
    ax.set_title(f"Comparaison Guessing Entropy par Clé — Config H ({PLATFORM})", fontsize=13)
    ...
    plt.tight_layout()
    path = os.path.join(RESULTS_DIR, f"comparison_keys_{PLATFORM}_configH.png")
